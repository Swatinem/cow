...
function cow(options) {
	...
	var toCopy = options.multiple && options.multiple || [options.single];
	...
	toCopy.forEach(copyObj);
	// then copy over all the props
	toCopy.forEach(copyProps);

	if (!copies.has(root))
		return [root, options.multiple || options.single];
	var leafCopies = toCopy.map(function (obj) { return copies.get(obj); });
	return [copies.get(root), options.multiple && leafCopies || leafCopies[0]];
